...
public sealed class CaiScorerTests
{
    ...
    public void Coverage_scales_the_effective_score()
    {
        ...
        var ch = Lens(Fold.Score(Bundle(Dim("D1", DimensionCategory.CodeQuality, 10, coverage: 0.5))), "codeHealth");
    ...
    public void Omitted_coverage_deserializes_as_full_coverage()
    {
        ...
        Assert.Equal(80.0, Lens(Fold.Score(bundle), "codeHealth").Score, 2);
    }
    ...
    {
        var withNoise = Fold.Score(Bundle(
        ...
        var clean = Fold.Score(Bundle(Dim("real", DimensionCategory.CodeQuality, 8))).Headline;
    ...
    {
        var s = Fold.Score(new EvidenceBundle
        {
            RubricVersion = "r",
    ...
    {
        var arch = Lens(Fold.Score(new EvidenceBundle
        {
            RubricVersion = "r",
    ...
    public void The_quality_bar_shifts_lens_bands_not_the_number()
    {
        ...
        Assert.Equal(68.0, Lens(Fold.Score(prod), "codeHealth").Score, 2);
        Assert.Equal(68.0, Lens(Fold.Score(poc), "codeHealth").Score, 2);
        Assert.Equal(Band.Fair, Lens(Fold.Score(prod), "codeHealth").Band);
        Assert.Equal(Band.Healthy, Lens(Fold.Score(poc), "codeHealth").Band);
    }
    ...
    public void Is_deterministic()
    {
        ...
        Assert.Equal(Fold.Score(b).Headline, Fold.Score(b).Headline);
    }
    ...
    public void Bands_at_the_canonical_thresholds(double score, Band expected) =>
        Assert.Equal(expected, ScoringParameters.Default.Bands.For(score));
    ...
    public void Golden_vector_reproduces_a_real_published_headline()
    {
        ...
        Assert.True(Fold.Verify(bundle).Reproduced);
    }
    ...
    public void Round_trips_through_json()
    {
        ...
        Assert.Equal(Fold.Score(b).Headline, Fold.Score(back).Headline, 6);
    }

    [Fact]
    public void Out_of_range_inputs_are_rejected()
    {
        Assert.Throws<ArgumentException>(() => Fold.Score(Bundle(Dim("D1", DimensionCategory.CodeQuality, 11))));
        Assert.Throws<ArgumentException>(() =>
            Fold.Score(Bundle(Dim("D1", DimensionCategory.CodeQuality, 5, coverage: 1.5))));
