...
/// <para>codeassuranceindex.info is served by the imprint CMS, so the standard's interactive proof tools (score a bundle,
...
public sealed class PublicCorsTests(RegistryApiFixture fx) : IClassFixture<RegistryApiFixture>
{
    ...
    {
        using var response = await PreflightAsync("https://codeassuranceindex.info");
        ...
        Assert.Contains("https://codeassuranceindex.info", allowed!);
    }
    ...
    public async Task A_real_cross_origin_post_carries_the_allow_header()
    {
        ...
        request.Headers.Add("Origin", "https://codeassuranceindex.info");
