...
public static class NoiseStandardEndpoints
{
    ...
    public static void MapNoiseStandard(this IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapGet("/api/noise/submissions/{submissionId}", (string submissionId, INoiseSubmissionStore store) =>
        {
        ...
        endpoints.MapGet("/api/noise/crowd/next", (string period, string raterId, INoiseFindingStore store) =>
        {
        ...
        endpoints.MapPost("/api/noise/crowd/answers", (CrowdAnswerRequest request, INoiseCostStore store) =>
        {
        ...
        endpoints.MapGet("/api/noise/findings/{findingId}", (string findingId, INoiseFindingStore store) =>
        {
        ...
        endpoints.MapGet("/api/noise/cost/{period}", (string period, INoiseCostStore store) =>
        {
        ...
        endpoints.MapPost("/api/noise/intent", (IntentRequest request, INoiseSubmissionStore store) =>
        {
        ...
        endpoints.MapGet("/api/noise/intent/{period}", (string period, INoiseSubmissionStore store) =>
        {
        ...
        endpoints.MapPost("/api/noise/disputes/{disputeId}/resolve",
            (string disputeId, DisputeResolutionRequest request, INoiseDisputeStore store) =>
        {
        ...
        endpoints.MapGet("/api/noise/rejudge/{period}", (string period, INoiseJudgingStore store) =>
        {
        ...
        endpoints.MapPost("/api/noise/rejudge/{period}", (string period, RejudgeRequest request, INoiseJudgingStore store) =>
        {
        ...
        endpoints.MapGet("/api/noise/published/{period}", (string period, INoisePublicationStore store) =>
        ...
        endpoints.MapGet("/api/noise/published", (INoisePublicationStore store) =>
    ...
    private static object RenderDisputes(INoiseDisputeStore store, string period)
    {
    ...
    private static object RenderRejudge(INoiseJudgingStore store, string period)
    {
        var judged = JudgedFindings(store, period);
        var seed = RejudgeSeed(period);
        var sample = Rejudge.SelectSample(seed, period, judged);
    ...
    /// </remarks>
    private static IReadOnlyList<string> JudgedFindings(INoiseJudgingStore store, string period) =>
    ...
    private static IReadOnlyDictionary<string, string> Settled(INoiseJudgingStore store, string period) =>
        store.ListResolutions(period)
            .Where(r => !string.IsNullOrWhiteSpace(r.Verdict))
